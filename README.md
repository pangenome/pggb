...
The 1D order is then broken into "blocks" which are "smoothed" using the [spoa](https://github.com/rvaser/spoa) partial order alignment (POA) algorithm.
...
Setting it higher can greatly increase runtime, because `spoa` is quadratic in the length of the longest sequence and graph that it aligns, but it also tends to produce cleaner resulting graphs.
